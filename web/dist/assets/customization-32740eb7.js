localStorage.hasOwnProperty("cursor")&&i(localStorage.getItem("cursor"));let s=document.querySelectorAll("#cursorOptions li");for(let e=0;e<s.length;e++)s[e].addEventListener("click",p);function p(e){i(e.target.closest("li").dataset.entryid)}async function i(e){if(localStorage.setItem("cursor",e),e){var t=document.getElementById("customCursorStyle");t||(t=document.createElement("style"),t.id="customCursorStyle",document.head.appendChild(t)),await fetch(location.protocol+"//"+location.host+location.pathname+"/actions/_portfolio-core/cursor/get-cursors",{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify({entryId:e,[window.Craft.csrfTokenName]:window.Craft.csrfTokenValue})}).then(n=>n.json()).then(n=>{t.textContent=`
            ...
            `})}else{var t=document.getElementById("customCursorStyle");t&&(t.textContent="")}}document.getElementById("file").addEventListener("change",e=>{const t=e.target.files[0],n=new FileReader;n.onloadend=()=>{const o=n.result.replace("data:","").replace(/^.+,/,"");localStorage.setItem("wallpaper",o),document.body.style.backgroundImage=`url(data:image/png;base64,${o})`},n.readAsDataURL(t)});localStorage.hasOwnProperty("wallpaper")&&(document.body.style.backgroundImage=`url(data:image/png;base64,${localStorage.getItem("wallpaper")})`);document.getElementById("openImgUpload").addEventListener("click",()=>{document.getElementById("file").click()});document.getElementById("removeBackground").addEventListener("click",()=>{document.body.style.backgroundImage="",localStorage.removeItem("wallpaper")});function u(e,t,n){let o=[],r=[5.2,116.14,66.66,117.18];for(let a=0;a<r.length;a++){let l=e,c=t,d=n;l=Math.floor(l/100*r[a]),c=Math.floor(c/100*r[a]),d=Math.floor(d/100*r[a]),o[a]=l+" "+c+" "+d}return o}localStorage.hasOwnProperty("BGColor")&&g(localStorage.getItem("BGColor"));function g(e){console.log(e);let t=document.getElementsByClassName("range"),n=document.getElementsByTagName("style")[0],o=u(t[0].value,t[1].value,t[2].value);n.innerHTML=`
